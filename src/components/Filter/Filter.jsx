...
const Filter = () => {
  ...
  const FilterCheck = memo((props) => {
    ...
    const handleCheck = (e, slug = props?.query?.slug) => {
      ...
      tempOption?.forEach((temp) => {
        // check for the slug
        if (temp.slug == slug) {
          if (temp?.values?.includes(e)) {
            // check if the option selected is already in values
            // remove the option and set checked = false
            if (temp.values)
              temp.values = temp?.values?.filter((val) => val !== e);
            setChecked(false);
          } else if (!temp?.values?.includes(e)) {
            //add the option and set checked = true
            if (temp.values) temp.values = [...temp.values, e];
            else temp.values = [e];
            setChecked(true);
          }
  ...
  return (
    <>
      <section
        className={
          "sideNav flex flex-col fixed w-full lg:w-2/5 h-full bg-white text-black items-start bottom-0 transition-all duration-300" +
          " " +
      ...
      >
        ...
        <section className='filterMenuWrapper w-full h-full flex overflow-y-scroll'>
          <section className='filterMenu w-2/5 h-full'>
          ...
          <section className='filterMenu w-3/5 h-full bg-gray-50'>
            {localFilter
